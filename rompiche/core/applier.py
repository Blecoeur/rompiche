def apply_suggestions(current_prompt, current_schema, suggestions):
    ...
    for suggestion in suggestions:
        ...
        if "prompt" in suggestion_lower:
            ...
            if any(keyword in suggestion_lower for keyword in ["format", "example", "iso 8601", "yyyy-mm-dd", "hh:mm"]):
                ...
                if "date" in suggestion_lower and "time" in suggestion_lower:
                    ...
                    updated_prompt = f"{current_prompt} Use ISO 8601 format for dates (YYYY-MM-DD)."
                elif "time" in suggestion_lower:
                    updated_prompt = f"{current_prompt} Use 24-hour format for times (HH:MM)."
            ...
            if any(keyword in suggestion_lower for keyword in ["edge-case", "ambiguous", "no time", "all-day"]):
        ...
        if "schema" in suggestion_lower:
            ...
            if "date" in updated_schema.get("properties", {}):
                if "format" in suggestion_lower or "iso 8601" in suggestion_lower or "yyyy-mm-dd" in suggestion_lower:
                    updated_schema["properties"]["date"]["format"] = "date"
                    updated_schema["properties"]["date"]["pattern"] = "^\\d{4}-\\d{2}-\\d{2}$"
                    
            if "time" in updated_schema.get("properties", {}):
                if "format" in suggestion_lower or "hh:mm" in suggestion_lower or "24-hour" in suggestion_lower:
                    updated_schema["properties"]["time"]["format"] = "time"
                    updated_schema["properties"]["time"]["pattern"] = "^([01]\\d|2[0-3]):[0-5]\\d$"
                    
            # Make time optional if suggested
            if any(keyword in suggestion_lower for keyword in ["optional", "no time", "all-day"]):
                if "time" in updated_schema.get("required", []):
                    updated_schema["required"] = [field for field in updated_schema["required"] if field != "time"]
...
if __name__ == "__main__":
    ...
    new_prompt, new_schema = apply_suggestions(current_prompt, current_schema, suggestions)

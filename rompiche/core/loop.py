...
def run_optimization_loop(max_iterations=5):
    ...
    for iteration in range(max_iterations):
        ...
        # 2. Ask the brain
        decision = get_brain_decision(metrics, current_prompt, current_schema, success_criteria)
...
def run_script_and_evaluate(prompt, schema):
    ...
    iteration = run_script_and_evaluate.iteration if hasattr(run_script_and_evaluate, 'iteration') else 0
    run_script_and_evaluate.iteration = iteration + 1
    return {"date_exact_match": 0.85 + iteration * 0.05, "title_similarity": 0.60 + iteration * 0.05}
